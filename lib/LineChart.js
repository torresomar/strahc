...
var LineChart = React.createClass({
  ...
  _setupDomains: function() {
    ...
    this._y.domain([minMax[0] - delta, minMax[1] + delta]);
  },
  _setupLineGen: function(){
    var chart = this;
    if(chart.props.areaChart) {
  ...
  _renderLines: function() {
    ...
    d3.select(svg)
      ...
      .append('path')
      .attr('class', 'line')
      .style('opacity', props.areaChart ?  0.5 : 1)
      .attr('stroke', function(d, i) {
        return (chart.props.colors[i]);
      })
      .attr('fill', function(d, i){
        return (props.areaChart ? chart.props.colors[i] : 'none');
      })
      .attr('d', function(d, i) {
        return chart.lineGen(d);
      });
